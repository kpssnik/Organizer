...
namespace kpssOrganizerServer_TEST
{
	...
	date	NUMERIC NOT NULL
        ...
        {
            string query = $"INSERT INTO Sessions(account_id, login_time, login_ip) VALUES({accId}, '{DateTime.Now}', '{ip}')";
            command = new SQLiteCommand(query, connection);
        ...
        public static Dictionary<string, string> GetSessionDictionary()
        {
            ...
            string query = "SELECT id, login_ip FROM Sessions";
            command = new SQLiteCommand(query, connection);

            SQLiteDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                temp.Add(reader["id"].ToString(), reader["login_ip"].ToString());
            }
            return temp;
